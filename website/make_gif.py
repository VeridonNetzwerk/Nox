...
def capture_via_cdp(ws, ms):
    ...
    # Set animation-delay via JS to seek
    js = f"""
    (function() {{
        var t = {ms};
        document.querySelectorAll('.nox-avatar, .nox-blob-path, .nox-droplet-a, .nox-droplet-b').forEach(function(el) {{
            var cs = getComputedStyle(el);
            var dur = parseFloat(cs.animationDuration);
            if (dur > 0) el.style.animationDelay = (-(t / 1000) % dur) + 's';
        }});
    }})();
    """
    cdp_send(ws, "Runtime.evaluate", {"expression": js}, msg_id=1)
...
def main():
    ...
    # Build transparent GIF with Floyd-Steinberg dithering
    p_frames = []
