...
class EnergyVAD:
    ...
    Provides the same is_speech(pcm_bytes, sample_rate) interface as webrtcvad.Vad.
    Uses RMS energy thresholding — less precise than webrtcvad but works on any
    Python version without compiled extensions.
    """

    # RMS energy thresholds per aggressiveness level (higher = less sensitive)
    _THRESHOLDS = {0: 200, 1: 350, 2: 500, 3: 800}

    def __init__(self, aggressiveness: int = 3):
        self.threshold = self._THRESHOLDS.get(aggressiveness, 800)
...
class VADRecorder:
    ...
    def record(self) -> "np.ndarray":
        ...
        if not self.is_available:
            logger.warning("VADRecorder unavailable: sounddevice=%s", _SD_AVAILABLE)
        ...
        try:
            if _VAD_BACKEND == "webrtc":
                ...
                vad = EnergyVAD(self.aggressiveness)
                logger.info("Using energy-based VAD fallback (webrtcvad not available)")
