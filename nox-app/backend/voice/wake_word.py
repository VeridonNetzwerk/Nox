...
class WakeWordListener:
    ...
    def pause(self) -> None:
        """Pause listening and release the microphone stream.

        Unlike just setting a flag, this actually closes the InputStream so
        that other consumers (e.g. VAD recorder) can open the device on
        Windows where exclusive access may be required.
        """
        self._paused = True
        logger.debug("Wake word listener paused")

    def resume(self) -> None:
        """Resume listening — the _run loop will reopen the stream."""
        self._paused = False
            ...
            try:
                with sd.InputStream(
                    ...
                    while self._running and not self._paused:
                        ...
                        # Debug: log scores above a low threshold to diagnose detection issues
                        for name, score in scores.items():
                            if score >= 0.1:
                                logger.debug(
                                    "Wake word score: %s = %.3f (threshold=%.2f)",
                            ...
                            if score >= self.threshold:
                                ...
                                # Cooldown to avoid double-trigger — wait until unpaused
                                time.sleep(2.0)
                                break

            except Exception as exc:
                logger.error("Wake word listener error: %s", exc, exc_info=True)
                time.sleep(1.0)  # Avoid tight loop on persistent error
