...
class VoiceManager:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # Wake Word — openWakeWord only (custom multi-language hey_nox.onnx model)
        ...
        # STT engine for transcription (after wake word triggers)
        ...
        # STT engine for voice transcription
        self.stt = stt_engine
        ...
        self._last_recording_from_wake = False
    ...
    def is_available(self) -> bool:
        ...
        return self.wake_word.is_available or (
            hasattr(self, 'stt') and self.stt.is_available
        )
    ...
    def _on_wake_detected(self, from_wake_word: bool = True) -> None:
        """Called by the wake word listener thread or manual voice_trigger.

        Args:
            from_wake_word: True if triggered by actual wake word detection,
                           False if triggered by UI button click.
        """
        ...
        if from_wake_word:
            ...
            play_wake_sound(self.tts.output_device)
        else:
            logger.info("Manual voice trigger – starting recording")
        self._start_recording(from_wake_word=from_wake_word)
    ...
    def _auto_listen_after_question(self) -> None:
        ...
        if text.rstrip().endswith("?"):
            ...
            self._start_recording(from_wake_word=False)
        else:
            self.wake_word.resume()

    def _start_recording(self, from_wake_word: bool = True) -> None:
        """Start the recording + transcription pipeline."""
        self._last_recording_from_wake = from_wake_word
        # Notify UI
        self._set_state(STATE_LISTENING)
        # Only call wake callback for actual wake word detections (not manual mic)
        if from_wake_word and self._on_wake:
            try:
                if self._loop and self._loop.is_running():
                    asyncio.run_coroutine_threadsafe(
    ...
    def _record_and_transcribe(self) -> None:
        ...
        try:
            ...
            # Play end-of-input sound if this was triggered by wake word
            if self._last_recording_from_wake and len(audio) > 0:
                play_end_sound(self.tts.output_device)
            ...
            if self._last_recording_from_wake:
            ...
            if self._on_transcript:
                try:
                    if self._loop and self._loop.is_running():
                        asyncio.run_coroutine_threadsafe(
                            self._async_transcript(transcript, self._last_recording_from_wake), self._loop
                        )
                    else:
                        logger.warning("No running event loop for transcript callback")
                except Exception as exc:
                    logger.error("Transcript callback error: %s", exc, exc_info=True)
        finally:
            self.wake_word.resume()

    async def _async_transcript(self, transcript: str, from_wake_word: bool = True) -> None:
        if self._on_transcript:
            result = self._on_transcript(transcript, from_wake_word)
            if asyncio.iscoroutine(result):
                await result
    ...
    def _is_tts_available(self) -> bool:
        """Check if the configured TTS engine is available."""
    ...
    def health(self) -> dict[str, Any]:
        ...
        elif hasattr(self.wake_word, "_model_name") and self.wake_word._model_name:
            wake_info["model"] = self.wake_word._model_name
        return {
            "wake_word": wake_info,
            "stt": {
            ...
            "tts": {
                "available": self._is_tts_available(),
