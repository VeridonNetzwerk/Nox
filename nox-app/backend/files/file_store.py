...
class FileStore:
    ...
    def search(
        ...
        1. Run FTS + filename search first (fast, ~5ms each)
        2. If enough results, skip semantic search entirely
        3. Only fall back to semantic search if FTS+filename found nothing
        ...
        """
        # Phase 1: FTS + filename (fast, no model loading)
        with self._lock:
            fts_results = self._fts_search(query, k * 3, folder_filter)
            fname_results = self._filename_search(query, k * 3, folder_filter)

        # Merge FTS + filename results
        ...
        # Phase 2: Only try semantic search if FTS+filename found nothing
        if not results and _ST_AVAILABLE:
            with self._lock:
                self._ensure_embedder()
                if self._embedder is not None:
    ...
    def _filename_search(
        ...
        Tries exact match first (uses idx_file_name index), then falls back
        to LIKE wildcard for partial matches.
        """
        try:
            # Try exact filename match first (fast, uses index)
            if folder_filter:
                rows = self._conn.execute(
                    "SELECT id, file_path, file_name, file_ext, content_text "
                    "FROM file_index "
                    "WHERE file_path LIKE ? AND file_name = ? "
                    "ORDER BY modified_time DESC LIMIT ?",
                    (folder_filter + "%", query, limit),
                ).fetchall()
            else:
                rows = self._conn.execute(
                    "SELECT id, file_path, file_name, file_ext, content_text "
            ...
            # Fall back to LIKE wildcard for partial matches
            if len(rows) < limit:
