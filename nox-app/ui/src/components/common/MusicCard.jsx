...
export default function MusicCard({ data, onOpen, onSetPlatform, locale }) {
  ...
  const PlatformButton = ({ pKey }) => {
    ...
    return (
      ...
      >
        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: isOpened ? "var(--nox-accent-fg)" : meta.color }} />
        <span style={{ color: isOpened ? "var(--nox-accent-fg)" : undefined }}>{meta.label}</span>
        {isOpened && (
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: "var(--nox-accent-fg)" }}>
