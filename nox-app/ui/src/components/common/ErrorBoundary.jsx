      ...
      return (
        <div className="h-full w-full flex flex-col items-center justify-center gap-4 p-6 nox-window-bg backdrop-blur-xl border border-nox-border rounded-2xl">
          ...
          <div className="text-center max-w-sm">
            ...
            <pre className="text-[10px] text-red-600 dark:text-red-300 bg-nox-surface rounded-lg p-2 max-h-32 overflow-y-auto text-left mb-3 break-all border border-red-500/20">
              {String(this.state.error).slice(0, 500)}
            </pre>
            <div className="flex gap-2 justify-center">
              <button
                onClick={this.handleReload}
                className="px-3 py-1.5 rounded-lg bg-nox-accent text-nox-accentFg text-xs hover:bg-nox-accentHover transition-colors"
              >
                Neu laden
              </button>
              <button
