...
<img src="img/nox-avatar.gif" width="200" alt="Nox Logo">
...
<p>
  ...
  <img src="https://img.shields.io/badge/Linux-Pop!_OS%20COSMIC-purple" alt="Linux">
...
| 💬 **Chat** | Streaming token-by-token responses via Ollama (Qwen3, Llama, Mistral, etc.) |
...
| 🔒 **100% Local AI** | All AI models run on your machine. Optional anonymous usage analytics (can be disabled in Settings) |
...
|-----------|---------|-------|
| OS | Windows 10/11 or Linux (Pop!_OS, Ubuntu) | Windows 10 build 19041+ / Linux with Python 3.11+ |
| Node.js | 20+ (LTS) | For development only |
| Python | 3.11+ | For development only (Linux: system python3) |
...
Nox automatically selects the best Ollama model based on your GPU's VRAM. **Qwen3** models are preferred for their strong multilingual support, reasoning capabilities, and tool-calling support.

| GPU VRAM | Recommended Model | Fallback |
|----------|------------------|----------|
| < 8 GB | Qwen3 4B | Qwen3 1.7B |
| 8–12 GB | Qwen3 8B | Qwen3 4B |
| 12–20 GB | Qwen3 14B | Qwen3 8B |
| ≥ 20 GB | Qwen3 14B or 32B | Qwen3 8B |

If no Qwen3 model is installed, Nox falls back to any available model with a matching size, or the first available model as a last resort. You can always switch models in **Settings**.
...
### Option B: Linux .deb Package (Pop!_OS / Ubuntu)

1. Download the latest `nox_0.1.0_amd64.deb` from [Releases](https://github.com/VeridonNetzwerk/Nox/releases)
2. Install with: `sudo apt install ./nox_0.1.0_amd64.deb` (or double-click in file manager)
3. Launch Nox from your application menu or run `nox` from terminal
4. On first run, Nox automatically:
   - Downloads Electron (if not already installed)
   - Installs Python dependencies via the setup screen
   - Installs Ollama (if not already installed)
   - Guides you through model selection and microphone setup
5. Done — start chatting or say **"Hey Nox"**

> **Linux Note**: Nox supports X11, Wayland, and COSMIC (Pop!_OS). On COSMIC/Wayland, screenshots use `cosmic-screenshot`, window management uses `cosmic-ext-window-helper`, and clipboard uses `wl-clipboard`.
...
| LLM | [Ollama](https://ollama.com) (Qwen3 preferred, auto-selected by VRAM) |
...
| `tts_model` | `de_DE-thorsten-medium` | TTS voice ID (Edge/Kokoro/Piper) |
| `tts_engine` | `piper` | TTS engine: edge, kokoro, or piper |
...
| Path | Content |
|------|---------|
| `%APPDATA%\Nox\config.yaml` | Configuration (Windows) |
| `~/.config/Nox/config.yaml` | Configuration (Linux) |
| `%APPDATA%\Nox\logs\` | Rotated log files (Windows) |
| `~/.config/Nox/logs/` | Rotated log files (Linux) |
| `%APPDATA%\Nox\data\nox.db` | SQLite: context + conversations (Windows) |
| `~/.local/share/Nox/data/nox.db` | SQLite: context + conversations (Linux) |
...
Output: `dist/Nox-Setup-0.1.0.exe`

### Linux .deb Build

```bash
cd nox-app/installers/linux
python3 build_deb.py
```

This builds a `.deb` package that includes:
- Backend Python source (dependencies installed on first run)
- `app.asar` (Electron app bundle with UI + main.js)
- Models (wake word, TTS)
- Bootstrap launcher (auto-downloads Electron on first run)
...
- **Nox-Installer** — NSIS `.exe` installer (Windows)
- **Nox-Unpacked** — Unpacked portable version (Windows)
- **Nox-Deb** — `.deb` package (Linux)
...
| Project | Role |
|---------|------|
| [Ollama](https://ollama.com) | Local LLM runtime — powers all chat and reasoning (Qwen3) |
